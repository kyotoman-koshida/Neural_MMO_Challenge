...
def checkpoint(flags, model, step: int):
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    network = Net()
    optimizer = torch.optim.Adam(network.parameters(), lr=3e-4, eps=1e-8)

    import pfrl

    actor_model_nn = Net()

    learner_model_nn = Net().to(device=flags.device)
    ...
    if flags.checkpoint_path is not None:
        ...
        learner_model_nn.load_state_dict(previous_checkpoint["model_state_dict"])
        print('チェックポイントからロードしました！')# 自作のチェックポイント読み込み確認メッセージ
    actor_model_nn.share_memory()
    actor_model_nn.load_state_dict(learner_model_nn.state_dict())

    learner_model = pfrl.agents.PPO(
        learner_model_nn,
        optimizer,
        gamma=0.9,
        gpu=-1,
